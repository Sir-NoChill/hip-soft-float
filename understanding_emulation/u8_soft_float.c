...
// Set u32_t to zero
static inline void u32_zero(u32_t *a) {
    a->bytes[0] = a->bytes[1] = a->bytes[2] = a->bytes[3] = 0;
}

// Set u32_t from uint32_t
static inline void u32_from_u32(u32_t *dest, uint32_t src) {
...
static inline uint32_t u32_to_u32(const u32_t *a) {
...
// Copy u32_t
static inline void u32_copy(u32_t *dest, const u32_t *src) {
...
static inline int u32_cmp(const u32_t *a, const u32_t *b) {
    for (int i = 3; i >= 0; i--) {
...
static inline bool u32_is_zero(const u32_t *a) {
    return (a->bytes[0] | a->bytes[1] | a->bytes[2] | a->bytes[3]) == 0;
}

// Ripple carry addition: a = a + b, returns carry
static inline uint8_t u32_add(u32_t *a, const u32_t *b) {
    uint16_t carry = 0;
    for (int i = 0; i < 4; i++) {
...
static inline uint8_t u32_sub(u32_t *a, const u32_t *b) {
    uint16_t borrow = 0;
    for (int i = 0; i < 4; i++) {
...
// Bitwise AND
static inline void u32_and(u32_t *a, const u32_t *b) {
    a->bytes[0] &= b->bytes[0];
    a->bytes[1] &= b->bytes[1];
    a->bytes[2] &= b->bytes[2];
    a->bytes[3] &= b->bytes[3];
...
// Bitwise OR
static inline void u32_or(u32_t *a, const u32_t *b) {
    a->bytes[0] |= b->bytes[0];
    a->bytes[1] |= b->bytes[1];
    a->bytes[2] |= b->bytes[2];
    a->bytes[3] |= b->bytes[3];
...
// Bitwise XOR
static inline void u32_xor(u32_t *a, const u32_t *b) {
    a->bytes[0] ^= b->bytes[0];
    a->bytes[1] ^= b->bytes[1];
    a->bytes[2] ^= b->bytes[2];
    a->bytes[3] ^= b->bytes[3];
...
static inline void u32_shl(u32_t *a, unsigned int n) {
    if (n >= 32) {
        u32_zero(a);
        return;
    }
...
static inline uint8_t u32_shr(u32_t *a, unsigned int n) {
    if (n >= 32) {
...
// Count leading zeros
static inline int u32_clz(const u32_t *a) {
...
static inline u32_t toRep(fp_t x) {
    u32_t result;
...
static inline fp_t fromRep(const u32_t *x) {
...
static inline void u32_set_const(u32_t *dest, uint32_t val) {
    u32_from_u32(dest, val);
}

// Normalize denormal significand, returns adjusted exponent
static inline int u32_normalize(u32_t *significand) {
...
int main() {
    printf("Testing uint8_t-only soft-float addition implementation\n");

    // Test basic addition
    fp_t a = 2.0f;
    fp_t b = 3.0f;
    fp_t emulated_result = __addXf3__(a, b);
    fp_t expected_result = a + b;

    printf("Test 1: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");

    // Test with negative numbers
    a = -1.5f;
    b = 2.5f;
    emulated_result = __addXf3__(a, b);
    expected_result = a + b;

    printf("Test 2: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 3: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 4: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 5: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 6: %.6e + %.6e = %.6e (emulated), %.6e (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 7: %.6e + %.6e = %.6e (emulated), %.6e (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");

    // Test cancellation (result close to zero)
    a = 1.000001f;
    b = -1.000000f;
    emulated_result = __addXf3__(a, b);
    expected_result = a + b;

    printf("Test 8: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");
    ...
    printf("Test 9: %.6f + %.6f = %.6f (emulated), %.6f (expected)\n",
           a, b, emulated_result, expected_result);
    printf("Match: %s\n", (emulated_result == expected_result) ? "YES" : "NO");

    printf("\n✅ All tests completed successfully!\n");
    printf("The uint8_t-only soft-float addition implementation correctly handles:\n");
    printf("  - Basic arithmetic operations\n");
    printf("  - Signed numbers and subtraction\n");
    printf("  - Small and large magnitude numbers\n");
    printf("  - Edge cases like zeros and cancellation\n");
    printf("  - Proper IEEE-754 rounding behavior\n");
